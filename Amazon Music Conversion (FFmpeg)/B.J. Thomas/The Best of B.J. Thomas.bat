...
ffmpeg -y -i "01 - Raindrops Keep Falling On My Head.mp3" ^
    -vf scale=512:512 -sws_flags bicubic ^
        cover_tmp.png
...
ffmpeg -y -i "01 - Raindrops Keep Falling On My Head.mp3" -i cover_tmp.png ^
    -map_metadata -1 -map 0:0 -map 1:0 ^
        -id3v2_version 3 -metadata artist="B.J. Thomas" -metadata album="The Best of B.J. Thomas" -metadata title="Raindrops Keep Falling on My Head" -metadata track="1/14" -metadata genre="Classical" -metadata language="English" -metadata copyright="© 2011, B.J. Thomas" -metadata encoder="FFmpeg" ^
            -c:1 png -disposition:1 attached_pic -c:a libmp3lame -ar 44100 -b:a 160k ^
