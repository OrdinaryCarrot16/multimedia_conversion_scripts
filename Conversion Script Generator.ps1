		...
		Get-ChildItem "*.mp3"
			do {
				...
				$songname=Read-Host -Prompt "Name of the Song"
					Add-Content -LiteralPath "$artist - $album [Generated].ps1" -Value "ffmpeg -i `"$filename`" -i cover_TMP.png -map_metadata -1 -map 0:0 -map 1:0 -id3v2_version 3 -metadata artist=`"$artist`" -metadata album_artist=`"$artist`" -metadata album=`"$album`" -metadata date=`"$date`" -metadata genre=`"$genre`" -metadata track=`"$TRACK_COUNTER/$track_amount`" -metadata title=`"$songname`" -c:1 png -disposition:1 attached_pic -af `"silenceremove=start_periods=1:stop_periods=1:start_threshold=-60dB:stop_threshold=-60dB,apad=pad_dur=5`" -c:a aac -aac_coder fast -ar 44100 -b:a 160k -movflags +faststart `"$songname.m4a`""
