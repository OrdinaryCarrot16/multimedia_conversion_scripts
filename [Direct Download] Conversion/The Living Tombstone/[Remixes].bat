...
ffmpeg -i "TombstoneDiscordRemix-Final.mp3" -i cover_TMP.png -map_metadata -1 -map 0:0 -map 1:0 -id3v2_version 3 -metadata artist="The Living Tombstone" -metadata title="Discord" -metadata genre="Techno | Pop" -metadata language="eng" -metadata date="2012-01-21" -metadata comment="Original song 'Discord' by 'Eurobeat Brony'." -c:1 png -disposition:1 attached_pic -af "loudnorm=I=-18:TP=-2:LRA=8:linear=true:measured_I=-6.7:measured_LRA=1.3:measured_TP=1.2:measured_thresh=-16.7" -c:a aac -aac_coder fast -ar 44100 -b:a 160k -movflags +faststart "Discord.m4a"  
